#!/usr/bin/env bash
# Set bash to 'debug' mode, it will exit on :
# -e 'error', -u 'undefined variable', -o ... 'error in pipeline', -x 'print commands',
set -e
set -u
set -o pipefail

fs=16000
n_fft=1024
n_shift=256

opts=
if [ "${fs}" -eq 22050 ]; then
    # To suppress recreation, specify wav format
    opts="--audio_format wav "
else
    opts="--audio_format flac "
fi

# Data prep related
train_type=multilingual # Use "en_us" or "multilingual".
token_type=byte

suffix=""
# Training the model only on the en_us single-speaker data
if [ "${train_type}" = en_us ]; then
    lang=en
    cleaner=none
    g2p=byte
    use_lid=false
    use_sid=false
    train_config=conf/train_vits.yaml
elif [ "${train_type}" = multilingual ]; then
    # Training the model on the whole dataset
    lang=noinfo
    cleaner=none
    g2p=byte
    use_lid=true
    use_sid=true
    if [ "${token_type}" = phn ]; then
        suffix="_phn"
    fi
    train_config=conf/train_vits_multilingual.yaml
else
    log train_type: "${train_type} is not supported."
    exit 1
fi

train_set=tr_no_dev${suffix}
valid_set=dev${suffix}
test_sets="dev${suffix} eval${suffix}"

inference_config=conf/decode_vits.yaml

# ./local/data.sh --train_type ${train_type}

./tts.sh \
    --lang ${lang} \
    --stage 2 \
    --ngpu 1 \
    --use_lid ${use_lid} \
    --use_sid ${use_sid} \
    --fs "${fs}" \
    --n_fft "${n_fft}" \
    --n_shift "${n_shift}" \
    --min_wav_duration 0.50 \
    --win_length null \
    --tts_task gan_tts \
    --dumpdir dump/vits16k \
    --expdir exp/vits16k \
    --feats_extract linear_spectrogram \
    --feats_normalize none \
    --token_type "${token_type}" \
    --cleaner "${cleaner}" \
    --g2p "${g2p}" \
    --train_config "${train_config}" \
    --inference_config "${inference_config}" \
    --inference_model latest.pth \
    --train_set "${train_set}" \
    --valid_set "${valid_set}" \
    --test_sets "${test_sets}" \
    --srctexts "data/${train_set}/text" \
    ${opts} "$@"
